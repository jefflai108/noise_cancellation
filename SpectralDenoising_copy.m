function y=SpectralDenoising_copy(filepath, Tms, a, b)
...
% quality
    ...
    %% locate the file
    filepath = '/Users/jefflai108/signals_systems/project2/section 3/violetNoise_sample.wav';
    Tms = 20; % ms per frame 
    [x,Fs] = audioread(filepath); % (40713,8000)
    ...
    for i=1:s_frames
        ...
        y_2 = [y_2, real(ifft(freq_dom',FFT_coeff,'symmetric'))'];
    end  

    audiowrite('violetNoise_sample_DENOISED.wav', y_2, Fs);
    ...
    subplot(3,1,1)
    plot(s_w)
    title('original Violet Noise signal');
    xlabel('time ms');
    ylabel('sample data');
    subplot(3,1,2)
    plot(n_w)
    ...
    subplot(3,1,3)
    plot(y_2)
    title('Denoised Violet Noise signal');
    xlabel('time ms');
    ylabel('sample data');
end 